...
def get_k_data(code=None, start='', end='',
                  ...
                  index=False,
                  retry_count=3,
                  pause=0.001,ip=None,agent=None):
    '''
    def get_k_data(code=None, start='', end='',
                      ktype='D', autype='qfq', 
                      index=False,
...
def _get_k_data(url, dataflag='',
    ...
    def _get_k_data(url, dataflag='',
                    ...
                    pause=0.001):
    ''' 
    #if和下面的else是新加的，如果要还原就删除这个if和下面的else（else中的全部内容删除），将if中的代码回退四格
    if ip is None:               
        for _ in range(retry_count):
                time.sleep(pause)
                try:
        ...
        for _ in range(retry_count):
                ...
                try:
                    ...
                    #request = Request(url)
                    lines = opener.open(url, timeout = 10).read()
                    if len(lines) < 100: #no data
                        return None
                except Exception as e:
                    print(e)
...
def bar(code, apis=None, start_date=None, end_date=None, ktype='D', asset='E', 
           adj = None,
           ma = [],
           volratio = False,
           retry_count = 3):
    ...
    for _ in range(retry_count):
        try:
            ...
            if asset == 'E' and adj is not None:
                df = adj_factor(code)
                if ktype in ct.KTYPE_LOW_COLS: 
...
def _ex_bar(code, api=None, start_date=None, end_date=None, ktype='D', asset='E', 
           ma = [],
           volratio = False,
           retry_count = 3):
    if api is None:
...
def get_instrument(xapi=None):
    ...
    data=[]
    for i in range(50): # range for python2/3
        ds = xapi.get_instrument_info(i * 1000, 1000)
        data += ds
        if len(ds) < 1000:
            break
...
if __name__ == "__main__":
    ...
    apis = cs.get_apis()
    api, xapi = apis
    df = bar('IF1710', apis=apis, start_date='2016-08-01', end_date='', ktype='d', asset='X')
#     
#     df = get_instrument(xapi)

#     df = get_security(api)
#     print(df.ix['2017-08-01 10:08:00' : '2017-08-01 10:12:00'])
#     df.to_csv('d:\\instruments.csv', encoding='utf8')
#     df = bar('002636', api=api, adj='hfq', ktype='1min')
#     print(bar('002636', api=api, adj='hfq', ktype='d'))
#     df = adj_factor('000636')
